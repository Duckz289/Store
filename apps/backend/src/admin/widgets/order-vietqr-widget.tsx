  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        <Heading level="h2">{t("vietqr.title")}</Heading>
      ...
      <div className="grid gap-4 px-6 py-4">
        <div className="flex gap-4">
          ...
          />
          <div>
            <Text size="small">{t("vietqr.reference", { value: vietqr.reference })}</Text>
            <Text size="small">{t("vietqr.amount", { value: vietqr.expected_amount })}</Text>
            <Text size="small">
              Receiver: {vietqr.account_name} · {vietqr.account_number}
            </Text>
            <Text size="small">
              {t("vietqr.expires", { value: new Date(vietqr.expires_at).toLocaleString() })}
            </Text>
            <Text size="small" className="text-ui-fg-subtle">
              {t("vietqr.evidenceWarning")}
            </Text>
          </div>
        </div>
        {!vietqr.captured_at ? (
          ...
          <Text size="small">{t("vietqr.capturedAt", { value: new Date(vietqr.captured_at).toLocaleString() })}</Text>
        )}
        {vietqr.observations.length ? (
          <div>
            <Text size="small" weight="plus">
              {t("vietqr.latestObservation")}
            </Text>
            <Text size="small">
