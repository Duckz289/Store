...
const createRepairCaseStep = createStep(
  ...
  async (input: CreateRepairCaseInput, { container }) => {
    ...
    const securityService =
      container.resolve<SecurityModuleService>(SECURITY_MODULE)
      ...
      async () => {
        ...
        try {
          ...
          for (let attempt = 0; attempt < 5; attempt += 1) {
            ...
            const existing = await repairService.listRepairCases({
              code: candidate,
            })
            if (!existing.length) {
              code = candidate
          ...
          for (const attachmentInput of input.attachments ?? []) {
            const uploadedFile = await fileService.retrieveFile(
              attachmentInput.file_id,
            )
            if (uploadedFile.url !== attachmentInput.file_reference) {
              throw new MedusaError(
                MedusaError.Types.INVALID_DATA,
                "REPAIR_ATTACHMENT_REFERENCE_INVALID",
              )
            }
            const attachment = await repairService.createRepairAttachments({
              case_id: repairCase.id,
              file_reference: attachmentInput.file_reference.trim(),
              storage_provider: "medusa-file",
              classification: "intake_photo",
              mime_type: attachmentInput.mime_type.trim().toLowerCase(),
              size_bytes: attachmentInput.size_bytes,
              checksum: attachmentInput.checksum.trim().toLowerCase(),
              uploaded_by: null,
            })
          ...
          await securityService.createAuditEvents(
            prepareAuditEvent({
              ...
              metadata: {
                ...
                attachment_count: createdAttachmentIds.length,
              },
            }),
          )
          ...
          const created = await repairService.retrieveRepairCase(
            repairCase.id,
            {
              relations: ["contact", "device"],
            },
          ...
          if (repairCase) {
            if (createdAttachmentIds.length) {
              await repairService
                .deleteRepairAttachments(createdAttachmentIds)
                .catch(() => undefined)
            }
            const children = await Promise.all([
              ...
              repairService.listRepairDeviceSnapshots({
                case_id: repairCase.id,
              }),
              repairService.listRepairContactSnapshots({
                case_id: repairCase.id,
              }),
            ])
            await repairService
            ...
            await repairService
              ...
              .catch(() => undefined)
            await repairService
              .deleteRepairCases(repairCase.id)
              .catch(() => undefined)
          }
          throw error
        }
